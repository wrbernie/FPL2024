...
def sort_manager_history(manager):
    ...
    filename = os.getcwd() + "/data/managers/" + manager + "/22/gw_history.csv"
...
def merge_picks(gw,manager):
    ...
    basefile = "data/managers/" + manager + "/22/gameweeks/"
    fullfile = basefile + 'gw' + str(gw) + 'picks.csv'

    dfp = pd.read_csv(fullfile)
    dfid = pd.read_csv('data/seasons/22/player_ids.csv')
    dfgw = pd.read_csv('data/seasons/22/gameweeks/gw' + str(gw) + '.csv')
